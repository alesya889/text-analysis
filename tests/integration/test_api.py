...
class TestAnalyzeEndpoint:
    ...
    def test_analyze_english_text(self, client):
        ...
        response = client.post(
            "/analyze", json={"text": "Hello world. This is a test.", "language": "en"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_analyze_russian_text(self, client):
        ...
        response = client.post("/analyze", json={"text": "Привет мир. Это тест.", "language": "ru"})
        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "success"
...
class TestAnalyzeBatchEndpoint:
    ...
    def test_analyze_batch_two_texts(self, client):
        ...
        response = client.post(
            "/analyze-batch", json={"texts": ["Hello world.", "This is another test."]}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "success"
...
class TestCaching:
    ...
    def test_different_texts_not_cached(self, client):
        ...
        response1 = client.post(
            "/analyze", json={"text": "First unique text for cache test", "language": "en"}
        )
        response2 = client.post(
            "/analyze", json={"text": "Second unique text for cache test", "language": "en"}
        )
...
class TestErrorHandling:
    ...
    def test_unsupported_language(self, client):
        ...
        response = client.post(
            "/analyze", json={"text": "Bonjour", "language": "fr"}  # Французский поддерживается.
        )
    ...
    def test_request_too_large(self, client):
        ...
        response = client.post("/analyze", json={"text": large_text, "language": "en"})
